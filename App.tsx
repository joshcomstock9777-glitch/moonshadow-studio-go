...
export default function App() {
  ...
  const editorAdapter = useRef(createEditorAdapter()).current;
  ...
  useEffect(() => {
    ...
    setSeats((previous) => previous.map((seat) => ({
      ...seat,
      status: pathSession.isActive && seat.id === activeSeat?.id
        ? 'thinking'
        : seat.enabled ? 'listening' : 'offline',
    })));
    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        <View style={styles.talkBar}>
          ...
          />
          <Pressable
            style={[styles.sendBtn, pathIsLoading && styles.sendBtnDisabled]}
            onPress={handleSend}
